    ...
    Mobileselect.prototype = {
        ...
        _appendOptionsList: function () {
            ...
            $.each(this.options, function (i, a) { 
                ...
                $.each(a.atts, function(k,v) {
                    attribs += k + ' = "' + v + '" ';
                });
                that.$listcontainer.append('<a href="#" class="mobileSelect-control" '+ b +' data-value="' + a.value + '" '+attribs+' >' + a.text + '</a>');
...
//                    var label = $t.parent().is('optgroup') ? $t.parent().attr('label') : false;
                    ...
                    //console.log($t[0].attributes);
                    if($t[0].attributes.length > 0) { 
                        $.each($t[0].attributes, function(j,b) {
                            aKey = b.name;
                            aValue = b.value;
                            tmpKeys[aKey] = aValue;
                        });
    ...
    $.fn.mobileSelect.defaults = {
        template: '<div><div class="mobileSelect-title"></div><div class="list-container"></div>',
